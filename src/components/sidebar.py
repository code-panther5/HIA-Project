        ...
        if st.button("+ New Analysis Session", use_container_width=True):
            if st.session_state.user and 'id' in st.session_state.user:
                ...
                try:
                    ...
                    # Direct session clearing fallback
...
def show_session_list():
    if st.session_state.user and 'id' in st.session_state.user:
        ...
        try:
            ...
            # Direct service call fallback if SessionManager is being stubborn
            if 'auth_service' in st.session_state:
                success, sessions = st.session_state.auth_service.get_user_sessions(st.session_state.user['id'])
