...
class ChatAgent:
    def __init__(self):
        # Cache embeddings model in session state to avoid reloading (~2-5s)
        if "_hf_embeddings" not in st.session_state:
            st.session_state._hf_embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
        self.embeddings = st.session_state._hf_embeddings
