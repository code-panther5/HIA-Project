...
class SessionManager:
    ...
    def init_session():
        ...
        if 'auth_service' not in st.session_state:
            ...
            # Restore session once on first init
            st.session_state.auth_service.try_restore_session()
        ...
        # Validate token — but skip if validated recently (within 5 min)
        if 'user' in st.session_state:
            last_val = st.session_state.get('last_validation')
            if last_val and (datetime.now() - last_val) < timedelta(minutes=5):
                return  # Session still valid, skip network call
    ...
    def clear_session_state():
        ...
        keys_to_keep = ['session_initialized', 'theme']
